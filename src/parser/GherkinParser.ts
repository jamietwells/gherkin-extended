...
type GherkinType = {
  ...
  NonWhitespaceOrAtSymbol: string
  Comment: Comment;
  BlankLine: BlankLine;
...
function GherkinParserFactory(indent: number): TypedLanguage<GherkinType> {
  ...
  return createLanguage<GherkinType>({
    ...
    NonWhitespaceOrAtSymbol: () => regexp(/[^\s@]/).desc('non-whitespace'),

    // Comments and blank lines
    Comment: (r) => seq(r.HorizontalWhitespaceCharacter.many(), string('#'), r.HorizontalWhitespaceCharacter.many()).then(r.NoNewLineContent).skip(r.End).desc('comment').map(comment => ({ type: 'Comment', comment })),
    ...
    DescriptionParser: (r) => {
      ...
      const line = r.IndentSame.then(r.NoNewLineContent.skip(r.End));
    ...
    // All things tags
    Tag: (r) => string(`@`).then(r.NonWhitespaceOrAtSymbol.many().tie()).map(r => ({ type: `Tag`, value: r })),
    Tags: (r) => r.IndentSame.then(r.Tag.sepBy1(r.HorizontalWhitespaceCharacter.atLeast(1))).skip(r.End).map(([tag, ...tags]) => [tag, ...tags]),
    ...
    FilledCell: (r) => string(`|`).then(r.HorizontalWhitespaceCharacter.many()).then(not(r.HorizontalWhitespaceCharacter.many().then(string(`|`).or(r.NL)), `end of cell`).atLeast(1).tie()).skip(r.HorizontalWhitespaceCharacter.many()),
    CellContent: (r) => alt(r.EmptyCell, r.FilledCell),
    TableLine: (r) => r.CellContent.many().skip(string(`|`)),
    SingleTableRow: (r) => r.TableLine.skip(r.End).map(cells => ({ type: "TableRow", cells })),
    Table: (r) => r.IndentSame.then(r.SingleTableRow).atLeast(2).map(rows => ({ type: 'Table', rows })),

    StepLineOrTableBlock: (r) => alt(...stepIdentifiers.map(word => r.IndentSame.then(string(word)).then(r.HorizontalWhitespaceCharacter.many()).then(r.ParameterizedLine).skip(r.End).desc(`${word} step`).map((text) => ({ type: `Step`, word, text }))), r.IndentMoreLookahead.chain(chainIndented(ri => ri.Table))),
    ...
    ExamplesBlock: (r) => seq(r.ExamplesTitle.skip(r.End), r.IndentMoreLookahead.chain(chainIndented(ri => ri.Table))).map(([title, table]) => ({ ...title, table: table })),
    BackgroundBlock: (r) => seq(r.BackgroundTitle.skip(r.End), r.IndentMoreLookahead.chain(chainIndented(ri => alt(ri.StepLineOrTableBlock, r.BlankLinesOrComments).many()))).map(([title, steps]) => ({ ...title, steps })),

    ScenarioOutlineBlock: (r) => seq(r.ScenarioOutlineTitle.skip(r.End), r.IndentMoreLookahead.chain(chainIndented(ri => seq(alt(ri.StepLineOrTableBlock, ri.BlankLinesOrComments).many(), seq(ri.ExamplesBlock.atMost(1), ri.BlankLinesOrComments.many()))))).map(([title, [steps, [examples, blankLines]]]) => ({ ...title, steps, examples: [...examples, ...blankLines] })) as any,
    FeatureBlock: (r) => seq(r.TagsOptional, r.FeatureTitle.skip(r.End), r.IndentMoreLookahead.chain(chainIndented(ri => seq(ri.DescriptionParser, alt(ri.ScenarioBlock, ri.ScenarioOutlineBlock, ri.BackgroundBlock, ri.RuleBlock).many())))).map(([tags, feature, [description, scenarios]]) => ({ ...feature, tags, ...description, scenarios })),
    RuleBlock: (r) => seq(r.TagsOptional, r.RuleTitle.skip(r.End).skip(r.BlankLinesOrComments.many()), r.IndentMoreLookahead.chain(chainIndented(ri => ri.ExampleBlock.many()))).map(([tags, title, examples]) => ({ ...title, tags, examples })),
    ExampleBlock: (r) => seq(r.TagsOptional, r.ExampleTitle.skip(r.End), r.IndentMoreLookahead.chain(chainIndented(ri => alt(ri.StepLineOrTableBlock, r.BlankLinesOrComments).many()))).map(([tags, title, steps]) => ({ ...title, tags, steps })),
    ScenarioBlock: (r) => seq(r.TagsOptional, r.ScenarioTitle.skip(r.End), r.IndentMoreLookahead.chain(chainIndented(ri => alt(ri.StepLineOrTableBlock, r.BlankLinesOrComments).many()))).map(([tags, title, steps]) => ({ ...title, tags, steps })),
